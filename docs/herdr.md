...
## Workspaces & Sessionizer (proyectos)

El picker `prefix+s` **escanea `~/workspace` en vivo** (`git_only=true`): todo repo git
bajo `~/workspace/{assets,personal,work}` aparece solo, sin índices ni scripts que sincronizar.
`wallpapers` vive fuera del workspace (`~/Pictures`), porque no es un proyecto;
`dotfiles` entra vía el symlink `assets/dotfiles → ~/.local/share/chezmoi` (creado por
`run_onchange_after_85-sync-assets`).

```text
~/workspace/{assets,personal,work}
├── assets/obsidian-notes      (1-pane LazyVim override)
├── assets/dotfiles  -> ~/.local/share/chezmoi   (LazyVim + shell 50/50)
├── personal/learning-path     (nv + terminal)
├── personal/sistemaVeterinario
├── personal/yordycg-portfolio
└── work/web-scrapping-basic
```
...
# 2. Listo: aparece en prefix+s (sin tocar config ni indices)
#    El picker escanea en el momento y lo detecta solo.
