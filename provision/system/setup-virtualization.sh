...
if [ -f /etc/fedora-release ]; then
    ...
    run sudo dnf install -y --skip-unavailable @virtualization virtio-win
elif [ -f /etc/arch-release ]; then
    log_info "→ Detectado Arch Linux. Instalando QEMU, libvirt y virt-manager..."
    run sudo pacman -S --noconfirm --needed qemu-desktop libvirt virt-manager dnsmasq iptables-nft
elif [ -f /etc/debian_version ]; then
    log_info "→ Detectado Debian/Ubuntu. Instalando QEMU, libvirt y virt-manager..."
    run sudo apt-get update -y -q
    run sudo apt-get install -y -q qemu-system-x86 libvirt-daemon-system libvirt-clients bridge-utils virt-manager
else
...
if getent group libvirt >/dev/null; then
    ...
    run sudo usermod -aG libvirt "$REAL_USER"
