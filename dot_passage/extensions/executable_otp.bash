#!/usr/bin/env bash
#
# pass-otp 1.1.1 ADAPTADO para passage (age) en lugar de pass (gpg).
# Origen: https://github.com/pass-extension/pass-otp (MIT)  /  otp.bash
#
# Adaptaciones: se sustituyó el backend GPG por age:
#   - $PREFIX/$path.gpg  ->  $PREFIX/$path.age
#   - $GPG -d/-e ...     ->  $AGE -d -i $IDENTITIES_FILE / $AGE -e ${AGE_RECIPIENT_ARGS[@]}
#   - set_gpg_recipients ->  set_age_recipients
#
# Requiere: oathtool (genera TOTP/HOTP).
# Activar con: PASSWORD_STORE_ENABLE_EXTENSIONS=true
#
# Uso: passage otp [--clip,-c] <pass-name>  |  passage otp append <pass-name>
...
cmd_otp_append() {
  ...
  while IFS= read -r line; do
    ...
    [[ -n "$contents" ]] && contents+=$'\n'
    contents+="$line"
  done < <($AGE -d -i "$IDENTITIES_FILE" "$passfile")
